<?php
class Admin_AgendaController extends KZ_Controller_Action
{
	...
	public function indexAction()
	{
		...
		$arrAgenda					= $objModelAgenda->getAgenda();
		...
		if($this->getRequest()->isPost()) {
			...
			if(empty($strName)) {
                ...
                if(empty($arrMatchingAgendaBySlug) && is_array($arrMatchingAgendaBySlug)) {
                    ...
                    $intInsertID		= $objModelAgenda->addAgenda($arrAgendaData);

                    if(isset($intInsertID) && is_numeric($intInsertID)) {
                        $strFeedback = base64_encode(serialize(array('type' => 'success', 'message' => 'Succesfully added agenda')));
                        $this->_redirect('/admin/agenda/content/id/'.$intInsertID.'/feedback/'.$strFeedback.'/#tab1');
                    } else {
                        // Return feedback
                        $this->view->feedback = array('type' => 'error', 'message' => 'Something went wrong trying to add the agenda');
                    }
                }
                else
                {
                    $this->view->feedback = array('type' => 'error', 'message' => 'An agenda item with this url already exists');
                }
