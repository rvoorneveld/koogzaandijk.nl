...
class KZ_View_Helper_Guestbook extends Zend_View_Helper_Abstract
{
    ...
    public function guestbook($booIsFrontend = false)
    {
        ...
        if (true === $booIsFrontend) {
            $arrLimit = ['count' => 5,
                         'offset' => 0];
        } else {
            $intPageNumber = $this->view->GetParam('pagina');
            if (null === $intPageNumber) {
                $intPageNumber = 1;
            }
            $arrLimit = ['count' => $intItemsOnPage,
        ...
        if (false === empty($arrGuestbookData) && true === is_array($arrGuestbookData)) {
            ...
            foreach ($arrGuestbookData as $intGuestbookKey => $arrGuestbookValues) {
                ...
                if ('gastenboek' === $this->view->getParam('controller')) {
